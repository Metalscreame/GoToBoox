...
func (p *poastgresCommentRepository) GetAllCommentsByNickname(nickname string) (comments []repository.Comment, err error) {
	rows, err := p.Db.Query("SELECT book_id, user_nickname, commentary, commentary_date  FROM comments where user_nickname = $1", nickname)
	if err != nil {
		return
	}
	defer rows.Close()
...
func (p *poastgresCommentRepository) GetAllCommentsByBookID(bookID int) (comments []repository.Comment, err error) {
	rows, err := p.Db.Query("SELECT user_nickname, commentary, commentary_date  FROM comments where book_id = $1", bookID)
	if err != nil {
		return
	}
	defer rows.Close()
...
func (p *poastgresCommentRepository) InsertNewComment(email, nickname, comment string, bookID int) (err error) {
	t := time.Now()
	_, err = p.Db.Query("INSERT INTO comments (user_nickname,user_email,commentary,commentary_date,book_id) values($1,$2,$3,$4,$5)",
